...
try {
  ...
  const { output } = await generateText({
    ...
    system: [
      ...
      "Allowed APIs: copicu.selection.ids/items, copicu.history.search(query,{limit,content}), copicu.history.get(id,{content}), copicu.history.update(id,patch), copicu.history.remove(id), copicu.clipboard.read(), copicu.clipboard.writeText(text), copicu.clipboard.writeItem(id), copicu.ai.respondMarkdown({ instruction, items, context }), copicu.commands.run('picker.open',{query,rememberPrevious,show,focus}), copicu.picker.open({query,rememberPrevious,show,focus}), copicu.picker.filter(query), copicu.picker.activate(id,options), copicu.picker.show(), copicu.picker.hide(), copicu.window.rememberPrevious(), copicu.window.focusPrevious(), copicu.input.paste({shortcut}), copicu.ui.toast/notify/alert/confirm/input/markdownOutput, copicu.log.info/warn/error/debug.",
