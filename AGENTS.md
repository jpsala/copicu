...
- Si JP dice `checkpoint`, `persistí estado` o pide guardar lo valioso antes de seguir, persistir solo valor durable en docs vivos sin cerrar sesion, sin handoff, sin thread nuevo, sin pedir `gol` y sin compactar salvo pedido explicito.
- En Pi, los comandos locales del OS (`/checkpoint-nudge`, `/os-status`, `/os-compact`, `/os-continuar`) estan documentados en `docs/topics/pi-agentic-os.md`.
