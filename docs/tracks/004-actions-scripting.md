...
- Pendiente para reabrir: validar `Ctrl+Alt+Shift+T` con Computer Use o tecla fisica; repetir `021`-`024` desde command palette con gesto UI real; repetir `npm run visual:check` porque la ultima corrida quedo inconclusa por timeout. Mantener datos sinteticos y no persistir payloads reales en logs/docs.
