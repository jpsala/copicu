...
- `Inbox`, `Saved searches`, `Capture modes` y `Tags` comparten esa superficie compacta; `Inbox` aplica el filtro estructurado canonico `is:inbox`. Las listas crecientes se abren desde el command palette o Settings.
...
- Invariante 2026-08-29: la primera geometria del feed debe aproximar la altura visible real antes de medir cada fila, reservar el gutter de scrollbar y propagar `width`/`height` conocidos a thumbnails. El decode de una imagen o la aparicion de la scrollbar no deben reenvolver y desplazar los items ya mostrados.
- Al montar en frío, la ausencia de una página inicial aún no significa historial vacío: mostrar carga hasta recibir el primer snapshot exitoso; el estado vacío sólo corresponde a una respuesta vacía. El frame del picker no anima su entrada para que una carga tardía no altere el aspecto entre aperturas frías y calientes.
- Al reabrir una sesión transitoria, ocultar el feed cacheado antes del primer frame visible y mantener una carga estática hasta que termine el primer refresh válido de esa sesión. El host anuncia `copicu://picker/hidden` en todas las rutas nativas de hide; el renderer también invalida al ocultar por comando propio. Un refresh anterior al hide no puede quitar la máscara, y un fallo conserva la máscara con error y Retry en vez de volver a mostrar ítems obsoletos. Keep-open y refresh por foco de una ventana que nunca se ocultó no aplican esta máscara.
