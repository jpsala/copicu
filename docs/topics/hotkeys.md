...
- Regla anti-regresion 2026-06-18: cualquier cambio en el hotkey del picker debe preservar que, al abrir desde otra app, el search reciba teclado sin hacer click ni llamar manualmente a `copicu_computer_use focus`. El oracle minimo es: enfocar app externa -> `open_picker` -> `type <token>` -> screenshot del picker mostrando el token en el input. Si el test llama `focus` antes de tipear, no valida este bug.
