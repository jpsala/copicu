...
- Los shortcuts editables desde ahi son `general.globalShortcut` para abrir picker, `picker.settingsShortcut` para abrir Settings desde el picker sin registro global, y `picker.pinToggleShortcut` para togglear pin/always-on-top del picker.
- Los shortcuts locales app-owned restantes se listan como read-only: `Ctrl+K`, `Ctrl+I`, `Enter`/`Shift+Enter`, `F2`/`Shift+F2`.
...
- `Ctrl+F2` abre el item de texto activo en el editor externo; `picker.externalEditorShortcut` permite registrar la misma ruta globalmente y queda deshabilitado por default hasta que el usuario elija una combinacion.
- El hotkey global no toca UI Tauri desde el callback nativo: despacha diferido al main thread y el renderer resuelve el item activo antes de iniciar la sesion externa.
- Confirmar o grabar `picker.externalEditorShortcut` persiste y registra esa combinacion inmediatamente; no depende del boton general `Save`. Settings debe refrescar el badge a `Registered` o revertir el valor si el backend reporta conflicto.
...
- Decision 2026-06-18: el hotkey global del picker vuelve a usar ruta con foco. El workaround no-activate de 2026-06-10 evitaba un bug visual de Codex/WebView2, pero generaba un fallo de producto mas grave: el picker podia verse al frente sin recibir teclado. La ruta default actual es `handle_global_shortcut` -> `spawn_toggle_main_window` -> `toggle_main_window` -> `show_main_window(... focus=true)`. La ruta no-activate queda solo como fallback diagnostico con `COPICU_PICKER_NO_ACTIVATE=1`.
- Regla anti-regresión: cualquier cambio en el hotkey debe preservar C0 con OMP `computer`: enfocar app externa exacta -> `desktop.press("ctrl+shift+.", { delivery: "foreground" })` -> `desktop.type("<token>", { delivery: "foreground" })` sobre el foco actual -> recién entonces capturar Copicu y confirmar token en search. Obtener/raise/targetear Copicu antes de escribir enmascara el bug; `desktop.windows()` o screenshot de target por sí solos no prueban keyboard-readiness.
...
- Hallazgo del incidente Codex 2026-06-10: desde Codex, cualquier intento probado de foco programatico posterior al hotkey (`window.set_focus`, foco diferido y click sintetico sobre search) volvia a producir la transparencia parcial de Codex. Cambiar el hotkey a `Ctrl+Alt+Shift+F12` no lo corrigio y ademas choco con automatizaciones AHK existentes. Esa ruta no-activate queda documentada solo como fallback de diagnostico porque sacrifica input listo-para-escribir.
- Estado actualizado 2026-06-18: el bug visual de transparencia de Codex queda conocido y no bloqueante frente a la prioridad de producto de que el picker sea keyboard-first. Si reaparece, usar temporalmente `COPICU_PICKER_NO_ACTIVATE=1` para comparar, pero no promover no-activate a default sin un reemplazo que garantice foco/input.
